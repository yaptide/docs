...
The editor JSON format is versioned. The `metadata.version` field tracks the schema version. The current version is **0.12**. The UI handles backward compatibility for older project files by migrating them on load.
